...
const second = <Input onChange={(e) => handelInput(e,'newPsw')}
                      key="newPsw"
                      style={{width: '300px'}}
                      prefix={<Icon type="lock" style={{color: 'rgba(0,0,0,.25)'}}/>}
                      type="newPsw"
...
export default class phoneChange extends React.Component {
    ...
    next = () => {
        ...
        if (this.state.current === 1 && newPsw) {
            const current = this.state.current + 1;
            this.setState({current});
            localStorage.setItem('currentUser', JSON.stringify({...JSON.parse(currentUser), password: newPsw}));
            userUpdate({...JSON.parse(currentUser), password: newPsw});
            return
        }
        else {
